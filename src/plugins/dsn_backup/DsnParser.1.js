...
define([
     ...
     */
    function DsnParser() {
        this.builder = new DsnDomainObjectBuilder();
        this.dsn = { data: {}, domainObjects: {}, stations: {} };
    }
    ...
    DsnParser.prototype.parseStationTag = function (stationElement) {
        ...
        this.dsn.stations[stationElement.getAttribute('friendlyName').toLowerCase()] = {};
    ...
    DsnParser.prototype.parseDishTag = function (dishElement) {
        ...
        location = this.getDishLocation(key);

        dish[key + '.name'] = dishElement.getAttribute('name');
        dish[key + '.azimuth'] = dishElement.getAttribute('azimuthAngle');
        dish[key + '.elevation'] = dishElement.getAttribute('elevationAngle');
        ...
        // if dish signals or targets, create folder and add to dish's composition

        // Build domain object for the dish
        dishKey = location + '.' + key;
        dishName = key.substring(0, 3).toUpperCase() + ' ' + key.substring(3);
        dishDomainObject = this.builder.buildDomainObject(dishKey, dishName, 'folder', location);
     ...
     */
    DsnParser.prototype.parseTimestampTag = function (element) {
        return element.textContent;
    };

    DsnParser.prototype.getDishLocation = function (name) {
        var dishNumber = name.slice(name.length - 2);
    ...
    DsnParser.prototype.parseXml = function (xmlDocument) {
        ...
        stations = this.dsn.stations;

        for (var station in stations) {
            sortedStations[station] = {};
            Object.keys(stations[station]).sort().forEach(function (key) {
                sortedStations[station][key] = stations[station][key];
            });
        }
